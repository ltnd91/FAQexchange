...
class AnswerCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if qs.exists():
            context['answers'] = qs.filter(question=context['question']).order_by('-followers', '-updated').distinct('followers')
            ...
            for ans in context['answers']:
                ...
                if ans in self.request.user.is_replying_answer.all():
                    ...
                    context['is_viewing_comment'] = Comment.objects.filter(answer=context['is_replying_answer']).order_by('-followers', '-updated').distinct('followers')
